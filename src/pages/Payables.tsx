...
import { PAYABLES } from '../constants';
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Obligaciones con Proveedores</h1>
            <p className="text-sm text-slate-500">Monitorea tus facturas pendientes de pago a proveedores.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total por Pagar</p>
            <h3 className="text-2xl font-black text-slate-900 mt-1">$2,500.00</h3>
            <p className="text-xs text-slate-500 mt-2">3 proveedores pendientes</p>
          </div>
          <div className="bg-white p-6 rounded-xl border border-red-100 shadow-sm bg-red-50/30">
            <p className="text-xs font-bold text-red-400 uppercase tracking-wider">Vencido</p>
            <h3 className="text-2xl font-black text-red-600 mt-1">$1,200.00</h3>
            <div className="flex items-center gap-1 mt-2 text-red-500">
              <AlertCircle className="size-3" />
              <span className="text-xs font-bold">Pago urgente requerido</span>
            </div>
          </div>
          <div className="bg-white p-6 rounded-xl border border-blue-100 shadow-sm bg-blue-50/30">
            <p className="text-xs font-bold text-blue-400 uppercase tracking-wider">Pagado (Mes)</p>
            <h3 className="text-2xl font-black text-blue-600 mt-1">$8,450.00</h3>
            <div className="flex items-center gap-1 mt-2 text-blue-500">
              <CheckCircle2 className="size-3" />
              <span className="text-xs font-bold">Flujo de caja estable</span>
            </div>
          </div>
        </div>
                ...
                {PAYABLES.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4">
                      <span className="text-sm font-bold text-slate-900">{item.supplier}</span>
                    </td>
